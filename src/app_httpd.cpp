...
static esp_err_t audio_status_handler(httpd_req_t *req) {
  ...
  char resp[10];
  if (audio_is_playing()) {
    strcpy((char *)resp, "true");
  } else {
    strcpy((char *)resp, "false");
  }
...
void startCameraServer() {
  ...
  httpd_uri_t audio_uri = {.uri = "/audio_playing",
                           .method = HTTP_GET,
  ...
  httpd_uri_t play_audio_uri = {.uri = "/play",
                                .method = HTTP_POST,
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &play_audio_uri);
